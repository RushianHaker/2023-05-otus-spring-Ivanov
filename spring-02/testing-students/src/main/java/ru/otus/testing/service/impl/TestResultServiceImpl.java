...
public class TestResultServiceImpl implements TestResultService {
    ...
    public TestResultServiceImpl(PrintService printService, @Value("${application.completeCount}") int completeCount) {
    ...
    public void checkTestResult(User user, int result) {
        if (result >= completeCount) {
            printService.print(user.getName() + " " + user.getLastName() +
